...
def add_or_update_event(event, calendar_event_id=None):
    ...
    Returns the Google Calendar event ID(s).
    """
    if not event.status == "published":
        logger.info(f"Skipping Google Calendar sync for event \
                    '{event}' because it is not published.")
        return None
    if not event.primary_tag:
        # No primary tag, so skip Google Calendar sync
        logger.info(f"Skipping Google Calendar sync for event \
                    '{event}' because it has no primary tag.")
        return None
    service = get_calendar_service()
    calendar_ids = [get_calendar_id(event.primary_tag.name)]
    if event.secondary_tag:
        calendar_ids.append(get_calendar_id(event.secondary_tag.name))
    event_ids = []
