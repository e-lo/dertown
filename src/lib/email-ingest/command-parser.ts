import type { MailgunPayload, EmailIntent } from './types';

const ANNOUNCEMENT_RE = /^(add\s+)?announcement[:\s]/i;
const SUBJECT_FWD_RE = /^(re|fwd?)[:\s]+/gi;
const URL_RE = /https?:\/\/[^\s<>"]+/;

export function parseIntent(payload: MailgunPayload): EmailIntent {
  ...
  // Prefer stripped-text (new content only); fall back to body-plain; last resort: strip HTML tags
  const rawBody =
  ...
  if (ANNOUNCEMENT_RE.test(normalizedSubject)) {
    ...
    const body = payload['body-plain'] ?? payload['body-html']?.replace(/<[^>]+>/g, ' ') ?? '';
