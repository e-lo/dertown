...
function runTests() {
  ...
  console.log(`Test Event: ${mockEventFromSupabase.start_date} at ${mockEventFromSupabase.start_time}`);
  console.log('='.repeat(60));
  console.log();
  ...
  try {
    ...
    const is5PM = (formattedTime.toLowerCase().includes('5:00') || formattedTime.toLowerCase().includes('5')) && 
  ...
  try {
    ...
    const has5PM = (formattedDateTime.toLowerCase().includes('5:00') || formattedDateTime.toLowerCase().includes('5')) && 
                   (formattedDateTime.toLowerCase().includes('pm') || formattedDateTime.toLowerCase().includes('p.m.'));
  ...
  try {
    ...
    if (!calendarEvent) {
      ...
      console.log(`Input: ${mockEventFromSupabase.start_date} ${mockEventFromSupabase.start_time} (locale)`);
      console.log(`Output start: ${calendarEvent.start}`);
      console.log(`Output end: ${calendarEvent.end}`);
      console.log(`Expected: UTC ISO string (e.g., "2025-12-03T01:00:00.000Z" if PST, or "2025-12-03T00:00:00.000Z" if PDT)`);
      console.log(`   Note: 5pm Pacific on Dec 2 should convert to Dec 3 in UTC (date boundary crossing)`);
      ...
      console.log(`When converted back to ${localeTimeZone}: ${pacificDateStr} at ${pacificTimeStr}`);
      ...
      const hasDec2 = pacificDateStr.includes('12/2') || pacificDateStr.includes('Dec 2') || pacificDateStr.includes('12/02') || pacificDateStr.includes('December 2');
      const has5PM = (pacificTimeStr.toLowerCase().includes('5:00') || pacificTimeStr.toLowerCase().includes('5')) && 
                     (pacificTimeStr.toLowerCase().includes('pm') || pacificTimeStr.toLowerCase().includes('p.m.'));
      ...
      if (isISOString && hasDec2 && has5PM) {
        ...
        console.log(`   FullCalendar with timeZone="${localeTimeZone}" should display: Dec 2 at 5:00 PM`);
        console.log(`   UTC date is Dec 3 (next day) which is correct for 5pm Pacific`);
        console.log(`   If FullCalendar shows a different date/time, check timeZone configuration\n`);
      } else {
        console.log(`❌ FAIL: UTC conversion incorrect. Expected Dec 2 at 5:00 PM Pacific, got ${pacificDateStr} at ${pacificTimeStr}`);
        console.log(`   This suggests the UTC conversion is wrong or FullCalendar timezone config is incorrect\n`);
        allTestsPassed = false;
      }
      ...
      if (utcDateOnly !== expectedUtcDate) {
        ...
        console.log(`   ⚠️  WARNING: UTC hour is ${actualUtcHour}, expected ${expectedUtcHourPST} (PST) or ${expectedUtcHourPDT} (PDT)`);
  ...
  try {
    ...
    if (calendarEvent && startDate.toISOString() === calendarEvent.start) {
      console.log('✅ PASS: parseEventTimesUTC produces same UTC conversion as transformEventForCalendar');
  ...
  try {
    ...
    const datesMatch = date1.month === date2.month && date1.day === date2.day && date1.dayOfWeek === date2.dayOfWeek;
